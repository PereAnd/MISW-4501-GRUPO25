...
def test_registro_perfil(client: FlaskClient):
    global id_perfil, id_empresa, id_proyecto, urlBackEnd
...
def test_elimina_perfil(client: FlaskClient):
    global id_perfil, id_empresa, id_proyecto, urlBackEnd, id_habilidad

    resp = client.post(
        '/empresa/' + str(id_empresa) + '/proyecto/' + str(id_proyecto) + '/perfil', json={'name': 'pruebaElimina','role': 'pruebaElimina','location': fake.city(),'years': fake.random_int(0, 40)})
    assert resp.status_code == 201
    assert resp.json.get('id')    
    id_perfil2 = resp.json.get('id')

    resp = client.delete(
        '/empresa/' + str(id_empresa) + '/proyecto/' + str(id_proyecto) + '/perfil/' + str(id_perfil2))
...
def test_actualiza_perfil(client: FlaskClient):
    global id_perfil, id_empresa, id_proyecto, urlBackEnd, name
...
def test_crea_conocimiento(client: FlaskClient):
    global id_perfil, id_empresa, id_proyecto, urlBackEnd, id_conocimiento
    resp = client.post(
...
def test_crea_idioma(client: FlaskClient):
    global id_perfil, id_empresa, id_proyecto, urlBackEnd, id_idioma
    resp = client.post(
...
def test_circuit_breaker(client: FlaskClient):
    ...
    tempEnv = os.environ["PERF_BACK_URL"]
    os.environ["PERF_BACK_URL"] = 'http://noexiste.com:5001'
    resp = client.get(
        '/empresa/' + str(id_empresa) + '/proyecto/' + str(id_proyecto) + '/perfil') 
    ...
    resp = client.get(
        '/empresa/' + str(id_empresa) + '/proyecto/' + str(id_proyecto) + '/perfil')
    assert resp.status_code == 503
    os.environ["PERF_BACK_URL"] = tempEnv   
    resp = client.get(
        '/empresa/' + str(id_empresa) + '/proyecto/' + str(id_proyecto) + '/perfil')
    assert resp.status_code == 503
